...
void geom_view_t::target_view(cam_tv_e target_view_src,
  ...
  case cam_tv_e::tv_x : {
    ...
    look_at = center;
    look_view_point = center + vector3<float>{static_cast<float>(1.2 * bbsize.x()), 0.0f, 0.0f};
    look_up = {0.0, 0.0, 1.0};
  ...
  case cam_tv_e::tv_y : {
    ...
    auto center = 0.5 * (m_ext_obs->aabb.max + m_ext_obs->aabb.min);
    look_at = center;
    look_view_point = center + vector3<float>{0.0f, static_cast<float>(1.2 * bbsize.y()), 0.0f};
    look_up = {0.0, 0.0, 1.0};
    need_to_update_camera = true;
    break;
  }
  ...
  case cam_tv_e::tv_z : {
    ...
    auto center = 0.5 * (m_ext_obs->aabb.max + m_ext_obs->aabb.min);
    look_at = center;
    look_view_point = center + vector3<float>{0.0f, 0.0f, 1.2f * bbsize.z()};
    look_up = {0.0, 1.0, 0.0};
    need_to_update_camera = true;
    break;
  }
  ...
  case cam_tv_e::tv_a : {
    auto center = 0.5 * (m_geom->cell.v[0] + m_geom->cell.v[1] + m_geom->cell.v[2]);
    look_view_point = m_pos.get_value() + center - 2.0f * m_geom->cell.v[0];
    look_at = m_pos.get_value()  + center;
    look_up = {0.0 , 0.0 , 1.0};
    need_to_update_camera = true;
    break;
  }
  ...
  case cam_tv_e::tv_b : {
    auto center = 0.5 * (m_geom->cell.v[0] + m_geom->cell.v[1] + m_geom->cell.v[2]);
    look_view_point = m_pos.get_value() + center - 2.0f * m_geom->cell.v[1];
    look_at = m_pos.get_value()  + center;
    look_up = {0.0, 0.0, 1.0};
    need_to_update_camera = true;
    break;
  ...
  case cam_tv_e::tv_c : {
    auto center = 0.5 * (m_geom->cell.v[0] + m_geom->cell.v[1] + m_geom->cell.v[2]);
    look_view_point = m_pos.get_value() + center - 2.0f * m_geom->cell.v[2];
    look_at = m_pos.get_value()  + center;
    look_up = {0.0, 1.0, 0.0};
    need_to_update_camera = true;
    break;
...
void geom_view_t::upd_atom(const int at_id, const vector3<float> &pos, bool emit_hs_rec) {
  if (!m_geom)
    return;
  if (emit_hs_rec)
    begin_recording(hs_doc_rec_type_e::hs_doc_rec_as_new_epoch);
...
void geom_view_t::upd_atom(const int at_id, const std::string &at_name, bool emit_hs_rec) {
  if (!m_geom)
    return;
  if (emit_hs_rec)
    begin_recording(hs_doc_rec_type_e::hs_doc_rec_as_new_epoch);
...
void geom_view_t::upd_atom(const int at_id, const std::string &at_name,
                           const vector3<float> &pos, bool emit_hs_rec) {
  if (!m_geom)
    return;
  if (emit_hs_rec)
    begin_recording(hs_doc_rec_type_e::hs_doc_rec_as_new_epoch);
...
void geom_view_t::copy_cell(geom_view_t &src, bool rebuild_tree) {
  ...
  for (int i = 0; i < 3; i++)
    if (m_geom->get_DIM() > i)
      m_geom->cell.v[i] = src.m_geom->cell.v[i];
...
void geom_view_t::load_from_json(json &data, repair_connection_info_t &rep_info) {
  ...
  if (auto val_itr = data.find(JSON_GV_ANIMATIONS); val_itr != data.end()) {
    ...
    for (auto &anim : val_itr.value()) {
      ...
      if (tmp_anim_rec.m_anim_type == geom_anim_e::anim_static)
        static_anim_found = true;
...
py::list geom_view_t::get_sel_atoms(int index_offset) {
  ...
  for (size_t i = 0; i < m_geom->nat(); i++)
    if (sels.find(i) != sels.end())
      ret.append(i+index_offset);
  return ret;
}
